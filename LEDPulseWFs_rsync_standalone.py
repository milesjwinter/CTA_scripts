...
def LEDPulseWaveforms(tester,module,frequency,trigger,runDuration, groupList=[1], numBlock=2, deadtime=12500, triggerDly=580, my_ip=std_my_ip, kNPacketsPerEvent=std_kNPacketsPerEvent,thresh=thresh, outputDir="output/", local_rsync=True, log_temp=True, temp_file='temperature_log.txt', temp_interval=30,wasted_time=60):
...
#		tester.EnableSoftwareTrigger(False)
		...
		if (trigger%2)==0: #for external trigger
			...
			#		module.WriteASICSetting("Thresh_{}".format(group), asic, 0, True) #sets the trigger threshold that has been determined in another test

			#module.WriteASICSetting("Thresh_{}".format(0), 0, int(thresh[0*4+0]), True) #sets the trigger threshold that has been determined in another test
...
#	ld.close()
	IO.stopData(listener, writer)
        ...
        if local_rsync==True:
            ...
            #cmd = "rsync -tvrq --remove-source-files --omit-dir-times %s %s %s"%(outFile, outLogFile, outputDir)
